...
- **Panels implemented**:
  - **Panel A — Pipeline Timeline (240 days)**: week-column grid (Sunday-aligned) showing each day's most advanced pipeline stage across all files.  Stages are **submitted** (blue `#3b82f6`) → **parsing** (amber `#f59e0b`) → **approval** (purple `#a855f7`) → **approved** (green `#16a34a`).  Empty days are light gray.  Once a file reaches a stage all subsequent days keep that stage (or higher); backward moves (failed, cancelled, returned to parsing) are ignored.  Black month-boundary lines are drawn where calendar neighbours fall in different months; open-ended for months that extend beyond the visible window.
  - **Panel B — Observed uploaders**: table of `Name | Email | Uploads | Last upload`.
  - **Panel C — Recent raw files**: last 20 `Asset` rows with status badge (`New`, `Processing`, `Review`, `Failed`, etc.), size, modified date, uploader, and source folder.
  - **Panel D — Approval queue**: last 5 `ParsedOutput` rows with `comparison_status="sent_for_approval"`, showing period, version, row count, spend, impressions, and age.
  - **Panel E — Approved history**: last 5 `ParsedOutput` rows with `comparison_status="approved"`, same columns plus a **Download** button linking to the existing `download_approved_output` view.
  - **Panel F — Activity stream**: last 20 `AssetEvent` rows with timestamp, file name, event type, from→to status transition, and message. Includes filter chips (`All`, `Discovered`, `Status`, `Parse`, `Approval`, `Cancelled`) that hide non-matching rows client-side.
- **Frontend wiring**:
...
- **Styling**: scoped CSS added to the existing `<style>` block for `.detail-panel`, `.upload-histogram`, and `.detail-table`, reusing base.html badge/table variables.
- **Tests** (in `pipeline_dashboard/tests/test_views.py`):

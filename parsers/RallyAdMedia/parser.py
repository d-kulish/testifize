...
def parse_file(source_path: Path, input_schema: dict[str, Any], output_columns: list[str]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for worksheet in input_schema.get("worksheets", []):
            ...
            validate_headers(sheet, worksheet, input_schema["selected_columns"])
...
def validate_headers(sheet: Any, worksheet: dict[str, Any], selected_columns: dict[str, str]) -> None:
    header_row = worksheet["header_row"]
    for field, expected_name in selected_columns.items():
...
def parse_date(value: object, row_number: int, sheet_name: str) -> str:
    ...
    if isinstance(value, str) and value.strip():
        text = value.strip()
        for date_format in ("%Y-%m-%d", "%m/%d/%Y", "%Y-%m-%d %H:%M:%S", "%d-%b-%Y", "%d-%b-%y"):
            try:

...
def parse_file(source_path: Path, input_schema: dict[str, Any], output_columns: list[str]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for table in input_schema.get("tables", []):
            ...
            date_col = column_letter_to_index(columns["date"])
            spend_col = column_letter_to_index(columns["spend"])
            impressions_col = column_letter_to_index(columns["impressions"])

            for row_number, row in enumerate(
                sheet.iter_rows(min_row=table["first_data_row"], max_row=sheet.max_row, values_only=True),
                start=table["first_data_row"],
            ):
                date_value = value_at(row, date_col)
                if not _is_valid_date_value(date_value):
                    continue
                ...
                totals["Spend"] += parse_decimal(value_at(row, spend_col), row_number, f"{table['name']} spend")
                totals["Impressions"] += parse_decimal(
                    value_at(row, impressions_col),
                    row_number,
